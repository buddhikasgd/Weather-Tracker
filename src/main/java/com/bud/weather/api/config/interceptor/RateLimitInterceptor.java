...
public class RateLimitInterceptor implements HandlerInterceptor {
    ...
    private boolean handleRateLimits(HttpServletResponse response, String apiKey) {
        ...
        if (probe.isConsumed()) {
            ...
            long waitForRefill = probe.getNanosToWaitForRefill() / 1000000000L;
            response.addHeader("X-Rate-Limit-Retry-After-Seconds", String.valueOf(waitForRefill));
            throw new TooManyRequestsException("Rate limit has exceeded!! Please Try again in " + waitForRefill + " seconds");
        }
    }

    private void validateApiKey(String apiKey) {
        if (!StringUtils.hasLength(apiKey)) {
            throw new MissingHeaderException("Missing Header: X-api-key");
        }
